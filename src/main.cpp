...
int main(int, char **) {
  ...
  DebugGUI::init(window, main_scale, wgpu_device, wgpu_surface_configuration);
...
#endif
  {
    ...
    const int drawDebugGuiReturn =
        DebugGUI::draw(surface_texture, width, height, ResizeSurface);

    if (drawDebugGuiReturn == -1) {
      abort();
    } else if (drawDebugGuiReturn == 1) {
      continue;
    }
...
static WGPUAdapter RequestAdapter(WGPUInstance &instance) {
  ...
  WGPUFuture future = wgpuInstanceRequestAdapter(instance, &adapter_options,
                                                 adapterCallbackInfo);
